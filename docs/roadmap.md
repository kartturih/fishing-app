...
MFS-017 (Assign Lure to Catch), MFS-018 (Lure Catalog UX Improvements), MFS-019 (Lure-Based Catch Statistics), MFS-020 (General Catch Statistics), and MFS-021 (Species Statistics) are all complete. MFS-017 let an angler assign an owned lure (referencing the catalog `LureVariant`, not the `TackleBoxEntry` — see MFS-017's Conceptual Relationship) to a `Catch`, displayed in Catch Details. MFS-018 followed it with a presentation-only reorganization of the Lure Catalog and Personal Tackle Box add flow: the browsing list groups by lure model instead of by variant, a new Lure Model Details view lists every color variant of a selected model, and the add-photo dialog no longer silently completes an add on dismissal. MFS-019 introduced the new Statistics feature and its first tab, Lure Statistics: two summary cards (most successful lure, most successful lure type), a per-lure catch-count list, and a per-lure-type catch-count breakdown, all computed live from existing catch and lure catalog data with no new persisted statistic, table, or migration. MFS-020 added the Statistics feature's first tab, Catches — moving Lure Statistics to the second tab position, unchanged in every other respect — presenting a Top 3 Largest Catches list ranked by weight (each entry opening the existing Catch Details view, MFS-014, and presented as a medal-bordered "Hall of Fame" after three rounds of post-testing presentation refinement), summary statistics (total catches, most caught species, rendered as equal-height cards), and a full per-species catch-count list. MFS-021 wired up the navigation MFS-020 deliberately left unimplemented: tapping a species row in the Species List now opens a pushed Species Statistics page for that species — a header (species name, total catches), a Record Catch card, and a full Catch List ordered by recorded weight, then catch date, then catch id, each entry opening the existing Catch Details view, with the page refreshing automatically on return from it. All five are architecture-reviewed, fully tested (575/575 automated tests), and physically verified on Android (see `docs/project-status.md`).

**Fishing Spot Statistics has been selected as the next milestone, to be specified as MFS-022.** It has not yet been drafted or approved — drafting begins immediately following this roadmap update. The Near-Term Roadmap (§3) has been reprioritized accordingly: Fishing Spot Statistics (MFS-022) is now first, followed by Catch Notes (MFS-023) and Catch Search & Filtering (MFS-024). A milestone's scope only becomes binding once its own MFS has been drafted and approved — this roadmap entry is a sequencing decision, not a specification.

---

## 3. Near-Term Roadmap

Proposed logical milestones after MFS-021, in the confirmed next-implementation order below. Only MFS-022 (Fishing Spot Statistics) is about to be drafted; MFS-023 (Catch Notes) and MFS-024 (Catch Search & Filtering) are prioritized to follow it, but neither is scoped yet — only an approved MFS defines actual feature scope (§1).

### 3.1 MFS-022 – Fishing Spot Statistics

* **Identifier:** Confirmed as the next milestone to specify, as MFS-022. Not yet drafted or approved — drafting begins immediately following this roadmap update.
* **Intent:** Extend the Statistics feature with a per-fishing-spot view, completing the natural third grouping axis alongside Species Statistics (MFS-021) and Lure Statistics (MFS-019). At a high level, expected capabilities include catches per fishing spot, the largest catch at that spot, total catches, a species breakdown, recent catches, and other useful fishing-spot-level statistics — computed live, the same way every existing Statistics view already is. Exact scope is intentionally left to MFS-022 itself, not defined here.
* **Depends on:** MFS-020 (General Catch Statistics) and MFS-021 (Species Statistics) — both complete. The repository/read-model/pushed-page pattern MFS-021 already established is expected to extend cleanly to a fishing-spot-scoped view.
...
### 3.2 MFS-023 – Catch Notes

* **Identifier:** Assigned as MFS-023, prioritized to follow Fishing Spot Statistics (MFS-022). Not yet drafted or approved.
* **Intent:** Let an angler attach a free-text note to a catch, enriching the existing structured catch record (species, weight, length, lure) with the context the project charter's own Problem Statement already asks for ("What has worked in similar conditions before?"). Becomes even more valuable once Fishing Spot Statistics (MFS-022) is available, since richer per-catch notes make reviewing a fishing spot's own catch history more useful.
* **Depends on:** MFS-009 (Catch Foundation) only — already complete; no hard dependency on MFS-022, but sequenced after it for the reason above.
* **Status:** Candidate, prioritized to follow MFS-022. Explicitly listed as an expected follow-up in MFS-009's Future Milestones. Not yet scoped, drafted, or approved.

### 3.3 MFS-024 – Catch Search & Filtering

* **Identifier:** Assigned as MFS-024, prioritized to follow Catch Notes (MFS-023). Not yet drafted or approved.
...
* **Status:** Candidate, prioritized to follow MFS-023. Not yet scoped, drafted, or approved.
...
* Weather information, water conditions, notes, lure information at the time — all superseded/being picked up incrementally by MFS-013 (photos, done), MFS-015/016/017 (lures, done), and the near-term Catch Notes (MFS-023) / Weather (§3.4) candidates above
